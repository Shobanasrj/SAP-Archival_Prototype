...
section[data-testid="stSidebar"] { background: linear-gradient(180deg, #0A1628 0%, #1A2B5E 60%, #0F3460 100%) !important; }
section[data-testid="stSidebar"] * { color: white !important; }
section[data-testid="stSidebar"] .stSelectbox > div > div,
section[data-testid="stSidebar"] .stTextInput > div > div > input { background: rgba(255,255,255,0.1) !important; border: 1px solid rgba(255,255,255,0.2) !important; color: white !important; border-radius: 8px !important; }
...
section[data-testid="stSidebar"] .stButton>button { background: rgba(232,160,32,0.2) !important; border: 1px solid #E8A020 !important; color: #E8A020 !important; border-radius: 8px !important; width:100%; }
#MainMenu, footer, header { visibility: hidden; }
.stDeployButton { display:none; }
.stTabs [data-baseweb="tab-list"] { background: white; border-radius: 12px; padding: 4px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); gap: 2px; margin-bottom: 1.2rem; }
.stTabs [data-baseweb="tab"] { border-radius: 8px !important; padding: 0.5rem 1rem !important; font-weight: 600 !important; font-size: 0.8rem !important; color: #64748B !important; background: transparent !important; border: none !important; }
.stTabs [aria-selected="true"] { background: linear-gradient(135deg, #1A2B5E, #2E86C1) !important; color: white !important; box-shadow: 0 2px 8px rgba(26,43,94,0.3) !important; }
.stTabs [data-baseweb="tab-panel"] { padding: 0 !important; }
.kpi-card { background: white; border-radius: 16px; padding: 1.2rem 1rem; box-shadow: 0 4px 20px rgba(0,0,0,0.06); border: 1px solid #E8EDF5; text-align: center; position: relative; overflow: hidden; transition: transform 0.2s; height: 100%; }
.kpi-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: var(--accent, linear-gradient(90deg,#1A2B5E,#2E86C1)); }
...
.section-hdr { display: flex; align-items: center; gap: 0.6rem; margin-bottom: 1rem; padding-bottom: 0.6rem; border-bottom: 2px solid #E8EDF5; }
.section-hdr h3 { margin:0; font-size:1rem; font-weight:700; color:#1A2B5E; }
.section-icon { width:32px; height:32px; border-radius:8px; background: linear-gradient(135deg,#1A2B5E,#2E86C1); display:flex; align-items:center; justify-content:center; font-size:16px; flex-shrink:0; }
.content-card { background: white; border-radius: 12px; padding: 1.2rem; box-shadow: 0 2px 12px rgba(0,0,0,0.05); border: 1px solid #E8EDF5; margin-bottom: 1rem; }
...
.ot-box { background: linear-gradient(135deg, #0A1628 0%, #1A2B5E 100%); border: 1px solid #E8A020; border-radius: 10px; padding: 1rem; margin: 0.5rem 0; }
...
.step-card { background: white; border-radius: 10px; border: 1px solid #E8EDF5; padding: 1rem; margin-bottom: 0.6rem; box-shadow: 0 2px 6px rgba(0,0,0,0.04); }
.stButton>button { border-radius: 8px !important; border: 1px solid #E2E8F0 !important; background: white !important; color: #1A2B5E !important; font-size: 0.78rem !important; font-weight: 500 !important; text-align: left !important; padding: 0.5rem 0.8rem !important; transition: all 0.15s !important; width: 100% !important; }
.stButton>button:hover { border-color: #1A2B5E !important; background: #F0F5FB !important; box-shadow: 0 2px 8px rgba(26,43,94,0.1) !important; }
...
.import-box { background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.15); border-radius:10px; padding:0.8rem; margin-top:0.5rem; }
.import-success { background:rgba(34,197,94,0.15); border:1px solid rgba(34,197,94,0.3); border-radius:8px; padding:0.6rem 0.8rem; margin-top:0.5rem; }
</style>
...
def parse_sap_export(file, source_type):
    """Parse various SAP export formats into normalised TABLE/SIZE_GB/ROWS dataframe"""
    try:
        ...
        # Normalise table name column
        for alias in ["TABLE_NAME","TABLE","TABNAME","OBJECT_NAME","TAB_NAME","MANDT_TABLE"]:
            if alias in df.columns:
                df = df.rename(columns={alias: "TABLE"}); break
        if "TABLE" not in df.columns:
            df = df.rename(columns={df.columns[0]: "TABLE"})

        # Normalise size column — handle MB, KB, GB
        size_found = False
        for col in ["SIZE_GB","SIZE_IN_GB"]:
            if col in df.columns:
                df["SIZE_GB"] = pd.to_numeric(df[col], errors="coerce")
                size_found = True; break
        if not size_found:
            for col in ["SIZE_MB","SIZE_IN_MB","USED_MB","ALLOCATED_MB"]:
                if col in df.columns:
                    df["SIZE_GB"] = pd.to_numeric(df[col], errors="coerce") / 1024
                    size_found = True; break
        if not size_found:
            for col in ["SIZE_KB","SIZE_IN_KB"]:
                if col in df.columns:
                    df["SIZE_GB"] = pd.to_numeric(df[col], errors="coerce") / (1024*1024)
                    size_found = True; break
        if not size_found:
            for col in ["SIZE","USED_SIZE","DISK_SIZE"]:
                if col in df.columns:
                    # Guess unit from values
                    vals = pd.to_numeric(df[col], errors="coerce")
                    median = vals.median()
                    if median > 100000:       df["SIZE_GB"] = vals / (1024*1024)  # KB
                    elif median > 500:         df["SIZE_GB"] = vals / 1024         # MB
                    else:                      df["SIZE_GB"] = vals                 # GB
                    size_found = True; break
        if not size_found:
            df["SIZE_GB"] = 0.0

        # Normalise rows column
        rows_found = False
        for col in ["ROWS","ROW_COUNT","NUM_ROWS","NROWS","ANZAHL","ANZ_ROWS","ROWS_K"]:
            if col in df.columns:
                multiplier = 1000 if "ROWS_K" in col else 1
                df["ROWS"] = pd.to_numeric(df[col], errors="coerce") * multiplier
                rows_found = True; break
        if not rows_found:
            df["ROWS"] = df["SIZE_GB"] * 500000  # rough estimate

        df["SIZE_GB"] = df["SIZE_GB"].fillna(0).clip(lower=0)
        df["ROWS"]    = df["ROWS"].fillna(0).clip(lower=0)
        df["TABLE"]   = df["TABLE"].astype(str).str.upper().str.strip()

        return df[["TABLE","SIZE_GB","ROWS"]].dropna(subset=["TABLE"]).reset_index(drop=True), None
...
with st.sidebar:
    ...
    st.markdown("### SOURCE SYSTEM IMPORT")

    source_type = st.selectbox("Export Source", [
        "Demo Data (no upload needed)",
        "DB02 / DBACOCKPIT",
        "TAANA — Table Analysis",
        "SE16 / SE16N — Direct Query",
        "AL11 / File System Export",
        "Custom CSV / Excel",
    ])

    # Show contextual help per source
    source_hints = {
        "DB02 / DBACOCKPIT":         "Transaction DB02 → Space → Tables → Export CSV",
        "TAANA — Table Analysis":    "Transaction TAANA → Run → Export results",
        "SE16 / SE16N — Direct Query":"SE16N → Table DBSTATC or M_TABLE_USED_MEMORY_VIEW → Export",
        "AL11 / File System Export": "AL11 → navigate to archive path → export file listing",
        "Custom CSV / Excel":        "Any table with TABLE_NAME, SIZE (MB/GB), ROWS columns",
    }
    if source_type in source_hints:
        st.markdown(f"""<div class="import-box">
        <div style="color:#E8A020;font-size:0.65rem;font-weight:700;letter-spacing:0.05em">HOW TO EXPORT</div>
        <div style="color:#B8D0E8;font-size:0.7rem;margin-top:0.3rem;line-height:1.5">{source_hints[source_type]}</div>
        </div>""", unsafe_allow_html=True)

    uploaded = None
    if source_type != "Demo Data (no upload needed)":
        uploaded = st.file_uploader(
            "Upload file",
            type=["csv","xlsx","txt","xls"],
            help="Upload your SAP export file. Accepted: CSV, Excel, TXT (tab-separated)"
        )

    st.markdown('<hr style="border:1px solid rgba(255,255,255,0.1);margin:1rem 0">', unsafe_allow_html=True)

    if has_ot:
        st.markdown(f"""<div style="background:rgba(232,160,32,0.15);border:1px solid rgba(232,160,32,0.4);border-radius:8px;padding:0.7rem">
...
if uploaded is not None and source_type != "Demo Data (no upload needed)":
    df_parsed, parse_error = parse_sap_export(uploaded, source_type)
    if df_parsed is not None and len(df_parsed) > 0:
        df_db = df_parsed
        label = f"📤 {source_type} · {uploaded.name} · {len(df_db)} tables"
    else:
        df_db = make_db()
        label = f"⚠️ Parse failed — using demo data"
else:
    df_db = make_db()

df_growth = make_growth()

# Show parse error at top if any
if parse_error:
    st.warning(f"⚠️ Could not fully parse upload: {parse_error}. Showing demo data. Check column names match: TABLE_NAME, SIZE_GB/MB, ROWS.")
...
<div style="background:linear-gradient(135deg,#0A1628 0%,#1A2B5E 50%,#1A4480 100%);
  ...
  <div style="position:absolute;top:-20px;right:-20px;width:200px;height:200px;background:rgba(232,160,32,0.08);border-radius:50%"></div>
  <div style="position:absolute;bottom:-40px;right:80px;width:120px;height:120px;background:rgba(46,134,193,0.1);border-radius:50%"></div>
  <div style="display:flex;justify-content:space-between;align-items:flex-start;position:relative">
    <div>
      <div style="font-size:0.7rem;color:#E8A020;font-weight:700;letter-spacing:0.15em;margin-bottom:0.4rem">CAPGEMINI · SAP DATA VOLUME MANAGEMENT</div>
      <h1 style="color:white;margin:0;font-size:1.7rem;font-weight:800;line-height:1.1">SAP Archival Intelligence</h1>
...
with t1:
    ...
    kpis = [
        ...
        ("🏆", f"{len(df_db)}",      "Tables Analysed",      "linear-gradient(135deg,#0369A1,#38BDF8)", "From source system"),
    ]
    cols = st.columns(5)
    ...
    with cl:
        st.markdown("""<div class="section-hdr"><div class="section-icon">📊</div><h3>Top Tables by Size</h3></div>""", unsafe_allow_html=True)
        top15 = df_db.nlargest(15,"SIZE_GB")
        fig = go.Figure(go.Bar(
            x=top15["SIZE_GB"], y=top15["TABLE"], orientation="h",
            marker=dict(color=top15["SIZE_GB"],colorscale=[[0,"#BDD7EE"],[0.5,"#2E86C1"],[1,"#1A2B5E"]],showscale=False,line=dict(width=0)),
    ...
    with cr:
        st.markdown("""<div class="section-hdr"><div class="section-icon">📈</div><h3>DB Growth Trend</h3></div>""", unsafe_allow_html=True)
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=df_growth["Month"],y=df_growth["GB"],fill="tozeroy",
            line=dict(color="#2E86C1",width=2.5),fillcolor="rgba(46,134,193,0.12)",
            mode="lines+markers",marker=dict(size=5,color="#1A2B5E")))
        fig2.update_layout(height=175,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                           margin=dict(l=0,r=0,t=0,b=0),showlegend=False,
                           xaxis=dict(showgrid=False,tickfont=dict(size=9)),
                           yaxis=dict(showgrid=True,gridcolor="#F1F5F9",tickfont=dict(size=9)))
        st.plotly_chart(fig2, use_container_width=True)

        st.markdown("""<div class="section-hdr" style="margin-top:0.8rem"><div class="section-icon">🍕</div><h3>By Module</h3></div>""", unsafe_allow_html=True)
        mods = {}
        ...
        if mods:
            fig3 = go.Figure(go.Pie(values=list(mods.values()),labels=list(mods.keys()),
                hole=0.55,textinfo="label+percent",
                marker=dict(colors=px.colors.sequential.Blues_r[:len(mods)]),textfont=dict(size=10)))
            fig3.update_layout(height=170,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                               margin=dict(l=0,r=0,t=0,b=0),showlegend=False)
            st.plotly_chart(fig3, use_container_width=True)

    st.markdown("""<div class="section-hdr"><div class="section-icon">🎯</div><h3>Archiving Opportunity by Object</h3></div>""", unsafe_allow_html=True)
    opp = []
    for k,obj in SAP_OBJECTS.items():
        gb = df_db[df_db["TABLE"].isin(obj["tables"])]["SIZE_GB"].sum()
        sc,pr = score(k,gb)
        opp.append({"Archive Object":k,"Module":obj["module"],"Size (GB)":round(gb,1),"Score":sc,"Priority":pr,"ILM":"✅" if obj["ilm"] else "—"})
...
with t2:
    ...
    for k,obj in SAP_OBJECTS.items():
        gb = df_db[df_db["TABLE"].isin(obj["tables"])]["SIZE_GB"].sum()
        sc,pr = score(k,gb)
        results.append({"key":k,"module":obj["module"],"desc":obj["desc"],"gb":gb,"sc":sc,"pr":pr,"ilm":obj["ilm"],"ot":obj["ot"],"complexity":obj["complexity"]})
    df_r = pd.DataFrame(results).sort_values("sc",ascending=False)
    hi=(df_r["pr"]=="HIGH").sum(); me=(df_r["pr"]=="MEDIUM").sum(); lo=(df_r["pr"]=="LOW").sum()

    st.markdown("""<div class="section-hdr"><div class="section-icon">🎯</div><h3>Archivability Scoring & Priority Matrix</h3></div>""", unsafe_allow_html=True)
    c1,c2,c3,c4 = st.columns(4)
    for col,v,l,c in [(c1,hi,"HIGH Priority","#EF4444"),(c2,me,"MEDIUM Priority","#F59E0B"),(c3,lo,"LOWER Priority","#22C55E"),(c4,(df_r["ilm"]==True).sum(),"ILM-Capable","#1A2B5E")]:
        col.markdown(f"""<div class="kpi-card"><div class="kpi-value" style="color:{c}">{v}</div><div class="kpi-label">{l}</div></div>""", unsafe_allow_html=True)
    ...
    with cl:
        ...
        for pr in ["HIGH","MEDIUM","LOW"]:
            sub = df_r[df_r["pr"]==pr]
            fig.add_trace(go.Scatter(x=sub["gb"],y=sub["sc"],mode="markers+text",
                text=sub["key"],textposition="top center",textfont=dict(size=8),name=pr,
                marker=dict(size=sub["gb"].apply(lambda x: max(8,min(30,x/3))),color=colors[pr],opacity=0.8,line=dict(width=1,color="white"))))
    ...
    with cr:
        ...
        for _,row in df_r.iterrows():
            ...
            st.markdown(f"""
            <div style="background:white;border-radius:8px;padding:0.6rem 0.8rem;margin-bottom:0.4rem;border:1px solid #E8EDF5;border-left:4px solid {clr}">
              ...
              <div class="prog-bar-wrap"><div class="prog-bar" style="width:{row['sc']}%;background:linear-gradient(90deg,{clr},{clr}88)"></div></div>
            </div>""", unsafe_allow_html=True)
    ...
    with cw1:
        st.markdown("""<div style="background:linear-gradient(135deg,#FEF2F2,#FFF5F5);border:1px solid #FECACA;border-radius:10px;padding:0.8rem;margin-bottom:0.5rem">
        <div style="color:#991B1B;font-size:0.75rem;font-weight:700;letter-spacing:0.05em">🔴 WAVE 1 — HIGH PRIORITY</div>
        <div style="color:#6B7280;font-size:0.7rem;margin-top:0.2rem">Execute first — maximum DB impact</div></div>""", unsafe_allow_html=True)
        for _,row in df_r[df_r["pr"]=="HIGH"].iterrows():
            st.markdown(f"""<div style="background:white;border-radius:8px;padding:0.5rem 0.7rem;margin-bottom:0.3rem;border:1px solid #FEE2E2;display:flex;justify-content:space-between">
            <div><b style="font-size:0.8rem">{row['key']}</b><br><span style="font-size:0.7rem;color:#94A3B8">{row['desc']}</span></div>
            <span style="font-weight:700;color:#EF4444;font-size:0.85rem">{row['gb']:.0f}GB</span></div>""", unsafe_allow_html=True)
    with cw2:
        st.markdown("""<div style="background:linear-gradient(135deg,#FFFBEB,#FFFDF0);border:1px solid #FDE68A;border-radius:10px;padding:0.8rem;margin-bottom:0.5rem">
        <div style="color:#92400E;font-size:0.75rem;font-weight:700;letter-spacing:0.05em">🟡 WAVE 2 — MEDIUM PRIORITY</div>
        <div style="color:#6B7280;font-size:0.7rem;margin-top:0.2rem">Execute after Wave 1 stabilises</div></div>""", unsafe_allow_html=True)
        for _,row in df_r[df_r["pr"]=="MEDIUM"].iterrows():
            st.markdown(f"""<div style="background:white;border-radius:8px;padding:0.5rem 0.7rem;margin-bottom:0.3rem;border:1px solid #FDE68A;display:flex;justify-content:space-between">
            <div><b style="font-size:0.8rem">{row['key']}</b><br><span style="font-size:0.7rem;color:#94A3B8">{row['desc']}</span></div>
            <span style="font-weight:700;color:#F59E0B;font-size:0.85rem">{row['gb']:.0f}GB</span></div>""", unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════
# TAB 3 — Object Catalogue
# ════════════════════════════════════════════════════════════════════
with t3:
    st.markdown("""<div class="section-hdr"><div class="section-icon">🏗️</div><h3>SAP Archive Object Reference Catalogue</h3></div>""", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    mod_f  = c1.multiselect("Module", sorted(set(o["module"] for o in SAP_OBJECTS.values())), default=list(set(o["module"] for o in SAP_OBJECTS.values())))
    ...
    for k,obj in SAP_OBJECTS.items():
        ...
        with st.expander(f"{icon} **{k}** — {obj['desc']}"):
            ...
            with col1:
                ...
                <div><b>Verify:</b> <code>{obj['tcode']}</code></div></div>""", unsafe_allow_html=True)
            with col2:
                st.markdown(f"""<div class="content-card">
                ...
                <div><b>Tables:</b> <code>{'` `'.join(obj['tables'][:3])}</code></div></div>""", unsafe_allow_html=True)
            with col3:
                st.markdown(f"""<div class="content-card">
                ...
                <div style="margin-top:0.4rem;font-size:0.75rem;color:#64748B">{obj['complexity']} complexity · {obj['impact']} impact</div></div>""", unsafe_allow_html=True)
            st.markdown(f"**Notes:** {obj['notes']}")
            if has_ot and obj["ot"]:
                if obj["ilm"] and has_ilm:
                    st.markdown("""<div class="ot-box"><h4>🔗 ILM + OpenText Path</h4><p>ILM-ADK files with retention metadata → OpenText via WebDAV/ILM 3.1. Retention enforced by OpenText. Legal hold supported.</p></div>""", unsafe_allow_html=True)
                else:
                    st.markdown("""<div class="ot-box"><h4>🔗 OpenText via ArchiveLink</h4><p>ADK files → OpenText via SAP ArchiveLink. No infrastructure change. Enable ILM later — files convertible with SAP ILM File Converter, no data loss.</p></div>""", unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════
# TAB 4 — AI Assistant
# ════════════════════════════════════════════════════════════════════
with t4:
    st.markdown("""<div class="section-hdr"><div class="section-icon">🤖</div><h3>SAP Archiving AI Assistant — Powered by Claude</h3></div>""", unsafe_allow_html=True)
...
Think like a senior consultant in a blueprint workshop. Format clearly with sections."""
    ...
    if not st.session_state.history:
        ...
        <div style="font-size:0.8rem;margin-top:0.3rem">Ask any question about SAP data archiving, ILM, OpenText integration, or S/4HANA readiness</div></div>"""
...
# ════════════════════════════════════════════════════════════════════
with t5:
    st.markdown("""<div class="section-hdr"><div class="section-icon">📋</div><h3>Archiving Remediation Playbook</h3></div>""", unsafe_allow_html=True)
    sel = st.selectbox("Select Archive Object", list(SAP_OBJECTS.keys()), format_func=lambda k: f"{k} — {SAP_OBJECTS[k]['desc']}")
    obj = SAP_OBJECTS[sel]
    gb  = df_db[df_db["TABLE"].isin(obj["tables"])]["SIZE_GB"].sum()
    sc,pr = score(sel,gb)
    clr = {"HIGH":"#EF4444","MEDIUM":"#F59E0B","LOW":"#22C55E"}[pr]
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#0A1628,#1A2B5E);border-radius:12px;padding:1.2rem 1.5rem;border:1px solid rgba(232,160,32,0.3);margin-bottom:1rem">
    ...
    if has_ot and obj["ot"]:
        if obj["ilm"] and has_ilm:
            st.markdown("""<div class="ot-box"><h4>🔗 ILM + OpenText Integration</h4><p>ILM-ADK files with embedded retention metadata → OpenText via WebDAV/ILM 3.1. Retention enforced natively. Legal hold and defensible deletion supported.</p></div>""", unsafe_allow_html=True)
        else:
            st.markdown("""<div class="ot-box"><h4>🔗 OpenText via ArchiveLink</h4><p>ADK files → OpenText via SAP ArchiveLink. No hardware change. Activate ILM later — existing files convertible with SAP ILM File Converter, zero data loss.</p></div>""", unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════
# TAB 6 — Methodology
# ════════════════════════════════════════════════════════════════════
with t6:
    st.markdown("""<div class="section-hdr"><div class="section-icon">ℹ️</div><h3>Methodology & Integration Architecture</h3></div>""", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    with c1:
        ...
        <li>Import DB02 / TAANA export → baseline the opportunity</li>
